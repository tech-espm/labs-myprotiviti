...
export = class Perfil {
	public static cachePerfis: Perfil[] = [];
	public static cachePerfisPorId: any = {};

	public id: number;
	public nome: string;
	public permissoes: any;

	public static async atualizarCache(): Promise<void> {
		let lista = await Perfil.listar();
		let cachePerfis: Perfil[] = [];
		let cachePerfisPorId: any = {};

		for (let i = 0; i < lista.length; i++) {
			let perfil = await Perfil.obter(lista[i].id);
			cachePerfis.push(perfil);
			cachePerfisPorId[perfil.id] = perfil;
		}

		Perfil.cachePerfis = cachePerfis;
		Perfil.cachePerfisPorId = cachePerfisPorId;
	}
	...
	public static async alterar(p: Perfil): Promise<string> {
		...
		if (p.id === 1)
			return "Não é possível editar o perfil \"Administrador\"!";
	...
	public static async excluir(id: number): Promise<string> {
		...
		if (id === 1)
			return "Não é possível excluir o perfil \"Administrador\"!";
