...
export = class Projeto {
	...
	public static async listar(): Promise<Projeto[]> {
		...
		await Sql.conectar(async (sql: Sql) => {
			...
			" ma.nome_matriz_servico,o.id_forma_contato,o.nome_forma_contato,rp.id_responsavel_proposta,rp.nome_responsavel_proposta,es.id_escritorio_lider,"+
			" es.nome_escritorio_lider,cc.id_cc_lider,cc.nome_cc_lider,p.valor_projeto, p.descricao_projeto"+
			" from projeto p, cliente c, segmento s, pursuit_team pu, matriz_servico ma, forma_contato o, responsavel_proposta rp, escritorio_lider es, cc_lider cc"+  
			" where p.id_cliente = c.id_cliente and p.id_segmento = s.id_segmento and p.id_matriz_servico = ma.id_matriz_servico"+
			" and p.id_pursuit_team = pu.id_pursuit_team and p.id_forma_contato = o.id_forma_contato"+
	...
	public static async criar(p: Projeto): Promise<string> {
		...
		await Sql.conectar(async (sql: Sql) => {
			try {
				await sql.query("insert into projeto (id_cliente, id_segmento, id_pursuit_team,id_matriz_servico,id_forma_contato,id_responsavel_proposta,id_escritorio_lider,id_cc_lider, valor_projeto, descricao_projeto) values (?,?,?,?,?,?,?,?,?,?,?)", [,p.id_cliente,p.id_segmento,p.id_pursuit_team,p.id_matriz_servico,p.id_forma_contato,p.id_responsavel_projeto,p.id_escritorio_lider,p.id_cc_lider,p.valor_projeto,p.descricao_projeto]);
			} catch (e) {
	...
	public static async alterar(p: Projeto): Promise<string> {
		...
		await Sql.conectar(async (sql: Sql) => {
			try {
				await sql.query("update projeto set id_cliente = ?, id_segmento = ?, id_pursuit_team = ?, id_matriz_servico = ?,id_forma_contato = ?,id_responsavel_proposta = ?,id_escritorio_lider = ?,id_cc_lider = ?,valor_projeto = ?, descricao_projeto = ? where id_projeto = " + p.id_projeto, [p.id_cliente,p.id_segmento,p.id_pursuit_team,p.id_matriz_servico,p.id_forma_contato,p.id_responsavel_projeto,p.id_escritorio_lider,p.id_cc_lider,p.valor_projeto,p.descricao_projeto]);
